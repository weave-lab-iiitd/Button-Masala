...
def getangle(x1,y1,x2,y2):
    print(x1,y1,x2,y2)
    result = np.degrees(math.atan((((y2-y1)/float(x2-x1))-1)/(1+((y2-y1)/float(x2-x1)))))
    print("jij",result)
    if result<0:
        result+=360
    # a=np.array([x1,y1])
...
def main():
    ...
    ret, img = cap.read()
    #Variables for Silhouette detection
	...
	#Silouhette detection Thresholding and Bakground Subtraction
	sil=gray-background
