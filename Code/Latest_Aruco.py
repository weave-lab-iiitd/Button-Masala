...
def getangle(x1,y1,x2,y2):
    ...
    try:
        ...
        result = np.degrees(math.atan(((float(y2-y1)/float(x2-x1+10))-1)/float(2+((y2-y1)/float(x2-x1+10)))))
    print("jij",result)
    if result<0:
        result+=360
    # a=np.array([x1,y1])
...
def main():
    ...
    #variables for Background Subtraction and Silhoutte
    background=initialise(capture)
	...
	#Background Subtraction and thresholding and removing noise
	sil=gray-background
...
#Code For Connecting  QR_Codes.
	...
	#needs code for setting an adaptive threshold
            threshold=120 #set an appropriate threshold for distance between finger and QRcentre
            if(ccc != None):
                center=ccc
                #print('yas')
	    ...
	    #centresl.append(find_centre(points))
            ...
            try:
                ...
                cv2.line(blank_image, (1500 - 500 - ll[3][0], ll[3][1]), (1500 - 500 - ll[0][0], ll[0][1]),
                         rep, 10)
                try:
                    cv2.line(blank_image, (1000-xa, ya), (1000-xa, ya), (180, 105, 255), 30)
                except:
                    pass
            ...
            for l in range(len(conn)-1):
                ...
                if len(conn) <= 2:
                    ...
                    colour = (160, 208, 255)
                cv2.line(blank_image, (1000-hist[conn[l].data][-1][0],hist[conn[l].data][-1][1]), (1000-hist[conn[l+1].data][-1][0],hist[conn[l+1].data][-1][1]), colour, 5)
...
#	if(len(arr)>=len(centres)):
	...
	#overlaying Images Aruco and silouhette
	sil=sil+blank_image

        cv2.imshow("input", fframe)
	cv2.imshow("Silouhette+Aruco",sil)
