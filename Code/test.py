...
def get_silhoutte(image):
	...
	# print(len(sil_arr[:5]))
        ...
        #cv2.resize(image,(240,1920))
        #cv2.resize(silhoutte,(240,4000))
        silhoutte = imutils.resize(silhoutte, width=900,height=900)
...
#Code For Connecting  QR_Codes.
	...
	#needs code for setting an adaptive threshold
        ...
        blank_image = cv2.resize(blank_image, (1080, 1920));
        arr = []
        lll = {}
        abra = False
        centresl=[]
	    ...
	    #centresl.append(find_centre(points))
            ...
            for l in range(len(conn)-1):
                ...
                if len(conn) <= 2:
                    ...
                    colour = (160, 208, 255)
                cv2.line(blank_image, (hist[conn[l].data][-1][0],hist[conn[l].data][-1][1]), (hist[conn[l+1].data][-1][0],hist[conn[l+1].data][-1][1]), colour, 5)
